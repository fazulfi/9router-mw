...
async function test(label, body) {
  ...
  console.log(label, 'STATUS:', resp.status, 'BODY:', text.slice(0, 200));
}

// Test 1: connectivity probe format (GY) with array content + system
await test('GY-probe:', {model: 'GLM-5.2', max_tokens: 16, stream: false, system: [{type: 'text', text: 'You are ZCode connectivity probe.'}], messages: [{role: 'user', content: [{type: 'text', text: 'hi'}]}]});
// Test 2: non-Plan oj with content as string
await test('oj-str:', {model: 'GLM-5.2', max_tokens: 16, temperature: 0.1, messages: [{role: 'user', content: 'hi'}]});
// Test 3: no system, content as array
await test('arr-content:', {model: 'GLM-5.2', max_tokens: 16, messages: [{role: 'user', content: [{type: 'text', text: 'hi'}]}]});
// Test 4: GLM-5-Turbo model
await test('GLM-5-Turbo:', {model: 'GLM-5-Turbo', max_tokens: 16, temperature: 0.1, messages: [{role: 'user', content: 'hi'}]});
