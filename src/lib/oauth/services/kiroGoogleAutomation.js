...
function isProxyAuthFailureMessage(message) {
  return /ERR_INVALID_AUTH_CREDENTIALS|407 Proxy Authentication Required|proxy authentication/i.test(String(message || ""));
}

function isProxyTunnelFailureMessage(message) {
  return /ERR_TUNNEL_CONNECTION_FAILED|ERR_PROXY_CONNECTION_FAILED/i.test(String(message || ""));
}
...
export async function runGoogleAccountAutomation({
  ...
  try {
    ...
    if (isProxyAuthFailureMessage(message)) {
      reportStep("proxy_auth_failed", `Proxy authentication failed while opening ${serviceLabel} OAuth`);
      return {
        status: "failed_proxy",
        step: "proxy_auth_failed",
        error: `Proxy authentication failed while opening ${serviceLabel} OAuth. Check the selected automation proxy username/password, proxy pool, or outbound automation proxy setting. If the password contains special characters, URL-encode them.`,
      };
    }
    if (isProxyTunnelFailureMessage(message)) {
      reportStep("proxy_tunnel_failed", `Proxy tunnel failed while opening ${serviceLabel} OAuth`);
      return {
        status: "failed_proxy",
        step: "proxy_tunnel_failed",
        error: `Proxy tunnel failed while opening ${serviceLabel} OAuth. Check/disable the selected automation proxy, proxy pool, or outbound automation proxy setting.`,
      };
    }
    throw error;
  }
