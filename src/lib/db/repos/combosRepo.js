...
export async function createCombo(data) {
  ...
  await db.run(`INSERT INTO combos(id, name, kind, models, createdAt, updatedAt) VALUES(?, ?, ?, ?, ?, ?)`,
...
export async function updateCombo(id, data) {
  ...
  const db = await getAdapter();
  let result = null;
  await db.transaction(async () => { const row = await db.get(`SELECT * FROM combos WHERE id = ?`, [id]);
  if (!row) return;
  const merged = { ...rowToCombo(row), ...data, updatedAt: new Date().toISOString() };
  await db.run(`UPDATE combos SET name = ?, kind = ?, models = ?, updatedAt = ? WHERE id = ?`,
