...
export async function disableModels(providerAlias, ids) {
  ...
  const db = await getAdapter();
  await db.transaction(async () => { const row = await db.get(`SELECT value FROM kv WHERE scope = ? AND key = ?`, [SCOPE, providerAlias]);
...
export async function enableModels(providerAlias, ids) {
  ...
  const db = await getAdapter();
  await db.transaction(async () => { if (!Array.isArray(ids) || ids.length === 0) {
