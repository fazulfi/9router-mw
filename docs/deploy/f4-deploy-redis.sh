#!/usr/bin/env bash
# Fase 4: deploy Redis shared-state release 0.5.35-mw.2
# Run as root on VPS. Does NOT touch foreign services (6379/6380).
set -euo pipefail

RELEASE_ID="${RELEASE_ID:-0.5.35-mw.2}"
...
EVIDENCE_DIR="${EVIDENCE_DIR:-/tmp/9router-mw-f4-evidence}"

mkdir -p "$EVIDENCE_DIR"
echo "=== F4 deploy release=${RELEASE_ID} ===" | tee "$EVIDENCE_DIR/00-start.txt"
...
text=open("/tmp/9router-mw-f4-evidence/10-health-samples.txt").read()
ids=[]
redis_ok=0
...
# models smoke
curl -sS -m 15 -o /tmp/f4-models.json -w 'models_http=%{http_code}\n' http://127.0.0.1:20128/api/v1/models | tee "$EVIDENCE_DIR/12-models.txt"
head -c 200 /tmp/f4-models.json | tee -a "$EVIDENCE_DIR/12-models.txt" || true
...
K6
  k6 run --summary-export "$EVIDENCE_DIR/14-k6-summary.json" /tmp/k6-f4-smoke.js 2>&1 | tee "$EVIDENCE_DIR/14-k6-smoke.txt" || true
else
  echo "k6 not installed" | tee "$EVIDENCE_DIR/14-k6-smoke.txt"
fi

echo "=== F4 deploy done ===" | tee "$EVIDENCE_DIR/99-done.txt"
