...
NGINX_SITE_AVAIL="/etc/nginx/sites-available/example.com"
NGINX_SITE_EN="/etc/nginx/sites-enabled/example.com"
...
# --- nginx site ---
if [[ -f "$RELEASE_DIR/docs/deploy/nginx-example.com.conf" ]]; then
 cp "$RELEASE_DIR/docs/deploy/nginx-example.com.conf" "$NGINX_SITE_AVAIL"
fi
...
curl -sk -m 10 -H 'Host: example.com' https://127.0.0.1/api/health | tee "$EVIDENCE_DIR/10-nginx-health.txt" || true
echo | tee -a "$EVIDENCE_DIR/10-nginx-health.txt"

# foreign services still up
docker ps --format 'table {{.Names}}\t{{.Status}}' | grep -E 'ggl-redis|app-redis|[CO-TENANT-A]|NAMES' | tee "$EVIDENCE_DIR/11-foreign-ok.txt" || true
