...
| Nginx `example.com` (local resolve) | health OK |
...
4. systemd unit + nginx site (SSL reuses [CO-TENANT-A] cert until CF DNS ready)

## Notes

- First nginx smoke with bare `Host:` + IP returned unauthorized; correct smoke uses `--resolve example.com:443:127.0.0.1`.
- DNS `example.com` still blocked (Cloudflare user action) — see `docs/deploy/DNS-CLOUDFLARE-BLOCKER.md`.
