...
describe("proxyAwareFetch — api.anthropic.com routing", () => {
  ...
  it("routes api.anthropic.com through native fetch (got-scraping disabled)", async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ id: "msg_test" }), {
      status: 200,
  ...
  it("falls back gracefully when native fetch handles anthropic requests", async () => {
    const originalFetch = globalThis.fetch;
